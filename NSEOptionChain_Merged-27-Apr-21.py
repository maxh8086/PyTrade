...
from historicVolatility import get_vix, url_to_json
...
gsh_oi.format('1', {'textFormat': {'bold': True}})
...
def main():
    ...
    # timeframe = 5
    timeframe = 1
    while time(9, 10) <= datetime.now().time() <= time(23, 30):
        ...
        if check:
            ...
            if not dailydata.empty:
                xl_oi.range("A1").options(index=False, header=True).value = dailydata[
                    ['expiryDate', 'Time', 'gamma_ce', 'vega_ce', 'theta_ce', 'delta_ce', 'estimate_ce', 'pChange_ce',
                     ...
                     'pChange_pe',
                     'estimate_pe', 'delta_pe', 'theta_pe', 'vega_pe', 'gamma_pe']]
                gd.set_with_dataframe(gsh_oi, dailydata[['expiryDate', 'Time', 'gamma_ce', 'vega_ce', 'theta_ce',
            ...
            if not mp_df.empty:
                xl_daily_oi_data.range("A1").options(header=True).value = mp_df[['Time', 'SpotPrice', 'MaxPain', 'PCR',
                                                                                 'vix', 'call_decay', 'put_decay',
                                                                                 'dvix', 'Dly CE', 'Dly CE-IV',
                                                                                 'Dly CE-Delta', 'Dly CE-ltp', 'Dly PE',
                                                                                 'Dly PE-IV', 'Dly PE-Delta',
                                                                                 'Dly PE-ltp', 'Live-ATM CE',
                                                                                 'Live-ATM CE-IV', 'Live-ATM PE',
                                                                                 'Live-ATM PE-IV',
                                                                                 'ATM CE', 'ATM CE-IV', 'ATM PE',
                                                                                 'ATM PE-IV', 'MaxPain-II']]
                gd.set_with_dataframe(gsh_daily_oi_data,
                                      mp_df[['Time', 'SpotPrice', 'MaxPain', 'PCR', 'vix', 'call_decay', 'put_decay',
                                             'dvix', 'Dly CE', 'Dly CE-IV', 'Dly CE-Delta', 'Dly CE-ltp', 'Dly PE',
                                             'Dly PE-IV', 'Dly PE-Delta', 'Dly PE-ltp',
                                             'Live-ATM CE', 'Live-ATM CE-IV', 'Live-ATM PE', 'Live-ATM PE-IV', 'ATM CE',
                                             'ATM CE-IV', 'ATM PE', 'ATM PE-IV',
                                             'MaxPain-II']])
            else:
            ...
            if not mp_df.empty:
                xl_weekly_oi_data.range("A1").options(header=True).value = mp_df[['SpotPrice', 'MaxPain', 'PCR',
                                                                                  'vix', 'call_decay', 'put_decay',
                ...
                gd.set_with_dataframe(gsh_weekly_oi_data, mp_df[['SpotPrice', 'MaxPain', 'PCR', 'vix',
