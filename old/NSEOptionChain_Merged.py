...
def fetch_oi(df, mp_df, dvix, wvix, mvix, vix, dayopen, prevclose, pchange):
    ...
    while tries <= max_retries:
        try:
            ...
            days = (expirydt - recentdt).days + (8 / 24)
            print(days)
            ...
            if (expirydt - recentdt).days == 0:
                ...
                if abs(pchange) > (dvix / 2):
                    ...
                    d_upper_value = maxpain2 * (1 + (dvx / 100))
                    d_lower_value = maxpain2 * (1 - (dvx / 100))
            ...
            mp_dict = {datetime.now().strftime("%H%M"): {
                ...
                'Dly sum ltp': oidata.loc[oidata['strikePrice'] == d_pe_i]['ltP_pe'].values[0] +
                                oidata.loc[oidata['strikePrice'] == d_ce_i]['ltP_ce'].values[0],
                'Wly CE': w_ce_i,
            ...
            try:
                if not df.empty:
                    df = df[
                        ['expiryDate', 'Time', 'gamma_ce', 'vega_ce', 'theta_ce', 'delta_ce', 'estimate_ce',
                         'pChange_ce',
                ...
                print("there is attribute error")
                pass
...
def main():
    ...
    while time(9, 16) <= datetime.now().time() <= time(23, 30):
        timenow = datetime.now()
        if timenow.minute / timeframe in slots:
            ...
            if not dailydata.empty:
                ...
                gd.set_with_dataframe(gsh_oi_live, dailydata[['expiryDate', 'Time', 'gamma_ce', 'vega_ce', 'theta_ce',
                                                              'delta_ce', 'estimate_ce', 'pChange_ce', 'change_ce',
                                                              'ltP_ce',
                                                              ...
                                                              'estimate_pe', 'delta_pe', 'theta_pe', 'vega_pe',
                                                              'gamma_pe']])
            else:
            ...
            if not mp_df.empty:
                xl_daily_oi_data.range("A1").options(header=True).value = mp_df[
                    ['Time', 'PreviousClose', 'Open', 'SpotPrice', 'MaxPain', 'PCR',
            ...
            if not mp_df.empty:
                xl_weekly_oi_data.range("A1").options(header=True).value = mp_df[
                    ['Time', 'PreviousClose', 'Open', 'SpotPrice', 'MaxPain', 'PCR',
                     'vix', 'call_decay', 'put_decay',
            ...
            if not df.empty:
                ...
                print("nextscan", nextscan, "RecentScan", recentdtm, "OI tm", dailydata.Time[0], "Recent tm",
                      recenttime)
                ...
                waitsecs = int(((forwardscan if nextscan < recentdtm or dailydata.Time[
                    0] < recenttime else nextscan) - recentdtm).seconds)
                print(waitsecs)
                if waitsecs > 0:
