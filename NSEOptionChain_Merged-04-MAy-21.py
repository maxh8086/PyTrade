...
xlwb = xw.Book(fullname="Option_chain_analysis.xlsx")  # this will create a new workbook
xl_oi = xlwb.sheets("sheet1")
xl_daily_oi_data = xlwb.sheets('Sheet2')
xl_weekly_oi_data = xlwb.sheets('Sheet3')
xl_daily_dump = xlwb.sheets("sheet4")
...
xl_oi.range('A:AZ').api.Delete()
xl_daily_oi_data.range('A:AZ').api.Delete()
xl_weekly_oi_data.range('A:AZ').api.Delete()
xl_daily_dump.range('A:AZ').api.Delete()
...
def fetch_oi(df, mp_df, dvix, wvix, mvix, vix, dayopen, prevclose, pchange):
    ...
    while tries <= max_retries:
        try:
            ...
            days = (expirydt - recentdt).days + (16/24)
            # print(days)
            ...
            if dt_today.weekday() == 3:
                ...
                # dvx = dvix * math.sqrt(5)
                # d_upper_value = dayprice * (1 + (dvx / 100))
                # d_lower_value = dayprice * (1 - (dvx / 100))
            else:
                # print(pchange)
                # dayprice = (dayopen if abs(pchange) > (dvix / 2) else prevclose)

                dvx = dvix * math.sqrt(days)
                d_upper_value = dayopen * (1 + (dvx / 100))
                d_lower_value = dayopen * (1 - (dvx / 100))
...
def main():
    ...
    while time(9, 15) > datetime.now().time() >= time(00, 00):
        ...
        if timenow.minute / timeframe in slots:
            ...
            waitsecs = int(((forwardscan if nextscan < datetime.now() else nextscan) - datetime.now()).seconds)
            if waitsecs > 0:
    ...
    while time(9, 15) <= datetime.now().time() <= time(15, 30):
        ...
        if timenow.minute / timeframe in slots:
            ...
            if not dailydata.empty:
                xl_oi.range("A1").options(index=False, header=True).value = dailydata[
                    ['expiryDate', 'Time', 'gamma_ce', 'vega_ce', 'theta_ce', 'delta_ce', 'estimate_ce', 'pChange_ce',
                     'change_ce', 'ltP_ce', 'Volume_ce', 'pchangeinOI_ce', 'changeinOI_ce', 'oI_ce', 'iV_ce',
                     'strikePrice',
                     'iV_pe', 'oI_pe', 'changeinOI_pe', 'pchangeinOI_pe', 'Volume_pe', 'ltP_pe', 'change_pe',
                     'pChange_pe',
                     'estimate_pe', 'delta_pe', 'theta_pe', 'vega_pe', 'gamma_pe']]
                gd.set_with_dataframe(gsh_oi_live, dailydata[['expiryDate', 'Time', 'gamma_ce', 'vega_ce', 'theta_ce',
                                                         ...
                                                         'Volume_ce', 'pchangeinOI_ce',
                                                         'changeinOI_ce', 'oI_ce', 'iV_ce', 'strikePrice', 'iV_pe',
                                                         'oI_pe', 'changeinOI_pe', 'pchangeinOI_pe',
            ...
            if not mp_df.empty:
                xl_daily_oi_data.range("A1").options(header=True).value = mp_df[['Time', 'PreviousClose', 'Open', 'SpotPrice', 'MaxPain', 'PCR',
                                                                                 'vix', 'call_decay', 'put_decay',
                                                                                 'dvix', 'Dly CE', 'Dly CE-IV',
                                                                                 'Dly CE-Delta', 'Dly CE-ltp', 'Dly PE',
                                                                                 'Dly PE-IV', 'Dly PE-Delta',
                                                                                 'Dly PE-ltp', 'Dly sum ltp', 'Live-ATM CE',
                                                                                 'Live-ATM CE-IV', 'Live-ATM PE',
                                                                                 'Live-ATM PE-IV',
                                                                                 'ATM CE', 'ATM CE-IV', 'ATM CE-Delta', 'ATM CE-ltp', 'ATM PE',
                                                                                 'ATM PE-IV', 'ATM PE-Delta', 'ATM PE-ltp', 'MaxPain-II']]
            ...
            if not mp_df.empty:
                xl_weekly_oi_data.range("A1").options(header=True).value = mp_df[['Time', 'PreviousClose', 'Open', 'SpotPrice', 'MaxPain', 'PCR',
                                                                                  'vix', 'call_decay', 'put_decay',
                                                                                  'Live-ATM CE', 'Live-ATM CE-IV',
                                                                                  'Live-ATM PE', 'Live-ATM PE-IV',
                                                                                  'wvix', 'Wly CE', 'Wly CE-IV',
                                                                                  'Wly CE-Delta', 'Wly CE-ltp',
                                                                                  'Wly PE', 'Wly PE-IV', 'Wly PE-Delta',
                                                                                  'Wly PE-ltp', 'Wly CE-Maxpain',
                                                                                  'Wly CE-MP-IV', 'Wly CE-MP-Delta',
                                                                                  'Wly CE-MP-ltp', 'Wly PE-Maxpain',
                                                                                  'Wly PE-MP-IV', 'Wly PE-MP-Delta',
                                                                                  'Wly PE-MP-ltp',
                                                                                  # 'Mly PE - I', 'Mly CE - I', 'Yearly PE - I', 'Yearly CE - I', 'Dly PE - II', 'Dly CE - II',
                                                                                  # 'Mly PE - II', 'Mly CE - II', 'mvix',
                                                                                  'MaxPain-II']]

                gd.set_with_dataframe(gsh_weekly_oi_data, mp_df[['Time', 'PreviousClose', 'Open', 'SpotPrice', 'MaxPain', 'PCR', 'vix',
            ...
            if not df.empty:
                ...
                #Write daily dump to to excel
                xl_daily_dump.range("A1").value = df
                xlwb.api.RefreshAll()
                ...
                # print((forwardscan if nextscan < datetime.now() else nextscan), datetime.now())
                ...
                # recenttime = (dtnow + timedelta(minutes=timeframe)).strftime('%H:%M')
                ...
                # waitsecs = int(((forwardscan if (nextscan < recentdtm and recenttime <= dailydata.Time[0]) else nextscan) - recentdtm).seconds)
