...
$pslog = $fpath + 'ps.log'
$process = 'python.exe'
While ((Get-Date -Format 'HH:mm') -gt '09:15' -and (Get-Date -Format 'HH:mm') -lt '23:59')
 {
  ...
  $File = $fpath + 'NSEOptionChain_Updated-AdHOC.py'
  #$File = $fpath + 'test.py'
   $value = Get-CimInstance Win32_Process -Filter "name = '$process'" | where {$_.CommandLine -match 'NSEOptionChain_Updated-AdHOC.py'}
