...
def fetch_oi(df_rw, mp_df_rw, dvix, wvix, mvix, vix, dayopen, prevclose, pchange):
    ...
    while tries <= max_retries:
        try:
            ...
            openingbell = time(9, 16) <= datetime.now().time() <= time(9, 25)
            ...
            scalper_ce = scalper_ce[scalper_ce != 0].dropna(subset=['iV_ce', 'gamma_ce'], inplace=False).head(1)
            scalper_ce1 = scalper_ce[scalper_ce != 0].dropna(subset=['iV_ce', 'gamma_ce'], inplace=False)
            scalper_pe = scalper_pe[scalper_pe != 0].dropna(subset=['iV_pe', 'gamma_pe'], inplace=False).head(1)
            scalper_pe1 = scalper_pe[scalper_pe != 0].dropna(subset=['iV_pe', 'gamma_pe'], inplace=False)
            print(scalper_ce1)
            print(scalper_pe1)
...
def main():
    ...
    while time(9, 16) <= datetime.now().time() <= time(23, 45):
        timenow = datetime.now()
        if timenow.minute / timeframe in slots:
            try:
